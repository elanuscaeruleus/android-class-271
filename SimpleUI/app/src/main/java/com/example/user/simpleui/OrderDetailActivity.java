...
public class OrderDetailActivity extends AppCompatActivity implements GeoCodingTask.GeoCodingRespone, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener, RoutingListener {
    ...
    public void onLocationChanged(Location location) {
        ...
        for(int i=0;i<points.size();i++)
        {
            if(i<points.size()-1)
            {
                ...
                if(location.getLatitude()>=minLat && location.getLatitude()<=maxLat && location.getLatitude()>=minLng && location.getLatitude()<=minLng)
                {
                    index=i;
                    break;
                }
